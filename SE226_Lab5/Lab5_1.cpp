...
string rep_with_upper(string word) {
    char target;
    target = word[rand() % word.length()];
...
string swap_letters(string word) {
    if(word.length() >= 4) {
        swap(word[0], word[word.length() - 2]);
        swap(word[1], word[word.length() - 1]);
    }
    return word;
}

int main() {
    srand(static_cast<unsigned>(time(NULL))); //in order to make sure that our "random" numbers aren't chosen randomly for only once

    int user_number;
    cout << "Enter a number between the range of 3~7: ";
    cin >> user_number;

    while(user_number < 3 || user_number > 7) {
        cout << "Please enter another number and make sure you meet the criteria." << endl;
        cin >> user_number;
    }
    ...
    string first = "ö";
    bool result = search_letter(password, first);

    //since it wasn't possible for me to cast from bool to string (const/dynamic/static_cast<basic_string<char>>(expression)) I used a print method with an if condition
